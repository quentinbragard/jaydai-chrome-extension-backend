...
async def verify_checkout_session(supabase: Client, session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if session.payment_status != "paid":
            if user_id:
                track_event(user_id, "Payment Failed", {"session_id": session_id})
        ...
        track_event(user_id, "Payment Succeeded", {"session_id": session_id})
        ...
        if 'session' in locals():
            ...
            if user_id:
                track_event(user_id, "Payment Failed", {"session_id": session_id, "error": str(e)})
